...
const StatsCard = ({ completedLessons, totalLessons, testCount, avgScore }: Props) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
    <h3 className="text-sm font-semibold mb-4" style={{ color: COLORS.primary }}>
      {CARD_TITLE}
    </h3>
    <div className="grid grid-cols-2 gap-4">
      <StatItem value={`${completedLessons}/${totalLessons}`} label={LABEL_LEKCIE} />
      ...
      />
      <StatItem
        value={testCount > 0 && avgScore != null ? (avgScore >= PASS_THRESHOLD ? '✓' : '✗') : '—'}
