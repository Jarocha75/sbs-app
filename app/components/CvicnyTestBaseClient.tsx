...
const CvicnyTestBaseClient = ({
  ...
  return (
    <main className="flex flex-col bg-page-bg min-h-screen pb-20">
      {/* Hero */}
      <div className="bg-primary py-10 px-4">
        <div className="max-w-2xl mx-auto">
          <Link
                ...
                const dotClass =
                  answers[q.id] !== undefined
                    ...
                    : i === currentIndex
                      ? "bg-accent"
                      : "bg-gray-300";
                return <div key={q.id} className={`rounded-full w-3 h-3 ${dotClass}`} />;
              })}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="px-7 py-5 flex flex-col gap-3">
              {(["A", "B", "C"] as QuizAnswer[]).map((letter) => {
                ...
                const btnClass = showGreen
                  ? "bg-success border-success text-white"
                  : showRed
                    ? "bg-danger border-danger text-white"
                    : !isAnswered && isSelected
                      ? "bg-blue-50 border-primary text-primary"
                      : `bg-white border-muted text-gray-700${isDimmed ? " opacity-[0.45]" : ""}`;
                ...
                return (
                  <button
                    ...
                    className={`flex items-start gap-4 text-left rounded-xl border-2 px-5 py-4 transition-all duration-200 disabled:cursor-default ${btnClass}`}
                  >
                    <span
                      className={`text-sm font-bold w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${labelClass}`}
                    >
                      {letter}
                    </span>
                    <span className="leading-relaxed text-[0.9375rem]">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-muted shadow-[0_-4px_20px_rgba(0,0,0,0.07)] z-50">
        <div className="max-w-2xl mx-auto px-4 flex items-center justify-between h-18">
          ...
          {isAnswered ? (
            ...
            <span className="text-sm text-muted-text">Vyberte odpoveď…</span>
          )}
        </div>
      </div>
    </main>
